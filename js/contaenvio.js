...
function enviarInfo(event){
    ...
    recepcion.setAttribute('href',`mailto:${envioForm.get('email')}?subject=nombre: ${envioForm.get('nombre')} password: ${envioForm.get('password')} apellido: ${envioForm.get('apellido')}email: ${envioForm.get('email')}&body=${envioForm.get('mensaje')}`);
    recepcion.click();
